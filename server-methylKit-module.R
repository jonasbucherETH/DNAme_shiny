methylKitServer <- function(id, dataValues, sharedValues) {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      dataValues$methylAll@sample.ids <- dataValues$sampleNames
      dataValues$methylDiff@sample.ids <- dataValues$sampleNames
      ...
      for(i in seq_along(dataValues$sampleNames)) {
        dataMethylRaw <- getData(dataValues$methylRawListDB[[i]])
      ...
      updateSelectInput(
        ...
        choices = dataValues$sampleNames,
        selected = dataValues$sampleNames[1]
      )
      ...
      output$methylKitTable <- DT::renderDataTable({
        dt_methylKit <- getData(methylDiff)

        dt_methylKit$pvalue <- round(as.numeric(dt_methylKit$pvalue), 3)
        dt_methylKit$qvalue <- round(as.numeric(dt_methylKit$qvalue), 3)
        dt_methylKit$meth.diff <- round(as.numeric(dt_methylKit$meth.diff), 3)
        
        dt_methylKit <- dt_methylKit[,-3]
        colnames(dt_methylKit) <- c("Chromosome", "Position", "Strand", "P-Value", "Q-Value", "Methylation Difference")
        
        # Create datatable with custom column names
        datatable(dt_methylKit, 
                  escape = FALSE, 
                  filter = "bottom", 
                  selection = "multiple")
      })
      
      resultsPCA <- PCASamples(dataValues$methylAll, obj.return = T) # uses prcomp using percent methylation matrix as input
      ...
      observeEvent( # Event PCA
        ...
        ignoreNULL = T, # default = TRUE
        {
          ...
          plotMethPCA <- pcaTableMeth %>%
            ...
            scale_color_manual(values = unlist(sharedValues$colourList[rownames(pcaTableMeth)])) +
            scale_fill_manual(values = unlist(sharedValues$colourList[rownames(pcaTableMeth)])) +
            # Add cross at origin
      ...
      observeEvent( # Event histograms (1)
        {
          ...
          input$color_methylation_histogram
          input$color_coverage_histogram
        },
        ignoreInit = F, # If TRUE, then, when the eventified object is first created/initialized, don't trigger the action or (compute the value). The default is FALSE.
        ignoreNULL = T, # default = TRUE
        {
          ...
          methylationHistogram <- methylationDF %>% 
            ...
            stat_bin(
              color = "black",
              # fill = "#3c8dbc",
              fill = input$color_methylation_histogram,
              geom = "bar",
              position = "stack",
              binwidth = 10,
              # breaks = seq(0,100,10),
          ...
          output$methylationHistogram <- renderPlot({
            ...
            # session$clientData[[paste0("output_", ns("methylationHistogram"), "_width")]] * 0.5
          # }
          )
          ...
          coverageHistogram <- coverageDF %>% ggplot(aes(x = log10_Coverage)) +
            stat_bin(
              color = "black",
              fill = input$color_coverage_histogram,
              # fill = "#28a745",
              geom = "bar",
              position = "stack",
              bins = 10,
          ...
          output$coverageHistogram <- renderPlot({
            ...
            # session$clientData[[paste0("output_", ns("coverageHistogram"), "_width")]] * 0.5
          # }
          )
          
      }) # close Event histograms
    }
  )
}

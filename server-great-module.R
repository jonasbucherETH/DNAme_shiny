...
greatServer <- function(id, greatResult, sharedValues) {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      # myPalette <- reactive({ sharedValues$myPalette })
      myColors <- colorRampPalette(colors = viridis::plasma(n = 7)[c(2, 4, 6)])(3)
      ...
      observe({
        ...
        observeEvent( # Event number 2
          ...
          ignoreNULL = F,
          {
            ...
            dataDotPlot$p_adjust[dataDotPlot$p_adjust==0] <- 0.01 
            ...
            output$dotPlot <- renderPlot({
              dotPlot
                ...
                session$clientData[[paste0("output_", ns("dotPlot"), "_width")]] * 0.5
            })
